...
#define SWAPT(a,b) temp=(a);(a)=(b);(b)=temp;
...
//conserved moment; usual values are m = 2or m = 4.
{
        ...
        float d,fac,sum,**a,*b;
        if (np < nl+nr+1 || nl < 0 || nr < 0 || ld > m||nl+nr < m) return(0);
        indx=new int[m+2];
        b=new float[m+2];
        a=new float*[m+2];
        for(int i=0; i<m+2; i++) a[i]=new float[m+2];
        ...
        if(!ludcmp(a,m+1,indx,&d))return(0);
        ...
        delete[] b;
        delete[]indx;
        for (int i=0; i< m+2; i++) delete[] a[i];
        delete[] a;
        return(1);
}
...
void multitap(int num_points, int nwin, double *lam, float npi, double *tapers,
